...
export default function WeatherCard() {
  ...
  function getWeatherImage(hour: number) {
    if (!weather) return "default_image.svg";
    const weatherCode = weather.weather_code[hour % 24];
  ...
  return (
    <div className="flex flex-col relative h-64 w-[400px] p-4 border-x-1 border-y-2 border-white/10 rounded-lg overflow-hidden">
      {weather ? (
        <>
          <img
            src={getWeatherImage(hour)}
            alt="Weather icon"
            className="absolute size-48 -top-8 -right-8"
          />
          <h1 className="text-3xl ">Auckland</h1>
          <p className="text-lg text-white/60">Tue, Aug 21</p>
          ...
          <div className="flex justify-evenly items-center pt-4">
            ...
            <div className="flex flex-col items-center">
              ...
              <p className="text-white/60">{weather.windspeed_10m[hour]}km/h</p>
            </div>
          </div>
        </>
      ) : (
